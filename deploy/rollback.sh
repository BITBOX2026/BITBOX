...
send_alert() {
  ...
  local message="$1" payload http_code webhook_url
  ...
  http_code="$(curl --silent --show-error --max-time 5 \
    ...
    "$webhook_url" || true)"
  if [[ "$http_code" != 2?? ]]; then
    log "failed to deliver rollback alert (HTTP ${http_code:-000})"
    command -v logger >/dev/null 2>&1 && logger -t bitbox-rollback "alert delivery failed (HTTP ${http_code:-000})"
  fi
}
