...
def _build_buses_from_route(data: dict) -> list[dict]:
    ...
    # 도보를 제외한 탑승 구간 수로 시간을 나눈다 (도보 포함 시 왜곡됨)
    transit_count = sum(
        1 for s in route_segments if s.get("vehicle_type") != "도보"
    ) or 1
    steps: list[dict] = []
    for seg in route_segments:
        vehicle_type = seg.get("vehicle_type", "")
        line = seg.get("line", "")
        is_bus = vehicle_type == "버스"
        is_walk = vehicle_type == "도보"
        seg_bus_number = line.replace("번", "").strip() if is_bus else ""
        seg_time = seg.get("time_min")
        duration = seg_time if seg_time is not None else round(total_time_min / transit_count)
        steps.append({
            ...
            "description": "도보" if is_walk else f"{line} {'탑승' if is_bus else '이용'}",
        })

    # 대표 버스 번호: bus_number 우선, 없으면 첫 번째 탑승 구간(도보 제외)에서 추출
    if bus_number:
        display_bus = bus_number
    else:
        first_transit_seg = next(
            (s for s in route_segments if s.get("vehicle_type") != "도보"), {}
        )
        first_line = first_transit_seg.get("line", "")
        display_bus = first_line.replace("번", "").strip()

    # 탑승 정류장: 도보 구간이 먼저 올 수 있으므로 첫 번째 탑승 step 기준으로 선택
    origin_stop = next(
        (s["fromStop"] for s in steps if s["type"] != "walk"),
        data.get("origin") or "",
    )
