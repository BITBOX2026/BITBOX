...
def _build_route_message(result: TransportResult) -> str:
    ...
    # 첫 번째 버스 탑승 정류장의 실시간 도착 시간 안내
    # 특수 상태("운행종료" 등)는 _format_arrival_time이 원문을 그대로 반환하므로
    # "운행종료 도착 예정입니다" 같은 비문이 생기지 않도록 제외한다.
    if result.arrival_time and result.bus_number and result.arrival_time not in _SPECIAL_ARRIVAL_TEXTS:
        parts.append(
            f"{result.bus_number}번 버스는 {_format_arrival_time(result.arrival_time)} 도착 예정입니다."
        )
