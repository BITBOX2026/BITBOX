...
async def _find_route_station_by_stop_text(
    ...
    getStationByName(arsId 확보)과 getStaionByRoute(노선 경유 목록)를 병렬 호출해
    5초 타임아웃 예산 내에 완료한다. arsId 매칭 실패 시 텍스트 매칭으로 폴백한다.
    """
    # 두 API를 병렬 호출 — 순차 호출 시 5초 타임아웃을 초과할 수 있음
    ars_id_candidates, payload = await asyncio.gather(
        _get_ars_id_candidates(stop_text),
        request_seoul_bus_payload(
            SEOUL_ROUTE_STATION_URL,
            {"busRouteId": bus_route_id},
            stage="노선 경유 정류소 조회",
        ),
    ...
    # arsId 기반 매칭 (텍스트 비교보다 정확함)
    if ars_id_candidates:
        for item in items:
            route_station = build_route_station(item)
            if route_station and route_station.get("ars_id") in ars_id_candidates:
                return route_station

    # arsId 매칭 실패 시 텍스트 매칭으로 폴백
...
async def _get_ars_id_candidates(stop_text: str) -> set[str]:
    """getStationByName API로 정류장명에 해당하는 arsId 목록을 조회합니다.

    실패 시 빈 set을 반환해 호출부가 텍스트 매칭으로 폴백할 수 있도록 합니다.
    """
    try:
        payload = await request_seoul_bus_payload(
            SEOUL_STATION_SEARCH_URL,
        ...
        for item in extract_items(payload):
            ars_id = first_item_value(item, ["arsId"])
            if ars_id and ars_id.strip():
                candidates.add(ars_id.strip())
        return candidates
    except Exception:
        logger.warning("getStationByName 조회 실패 — 텍스트 매칭으로 폴백: stop=%s", stop_text)
        return set()
